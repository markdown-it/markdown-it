...
## General considerations for plug-ins.

1. Try to find the right place for your plug-in rule:
  - Will it conflict with existing markup (by priority)?
    - If yes - you need to write an in-line or block rule.
  ...
  - Remember that token morphing in core chains is always more simple than writing
    block or in-line rules, if you don't copy existing ones. However,
    block and in-line rules are usually faster.
  ...
  - Plug-ins should not require the `markdown-it` package as dependency in `package.json`.
    ...
    passed on plug-in load. See properties of main class and nested objects.
2. Search existing
   [plug-ins](https://www.npmjs.org/browse/keyword/markdown-it-plugin)
